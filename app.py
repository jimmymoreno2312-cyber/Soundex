...
def create_list():
    ...
    # Read details of submitted list
    data = request.get_json(silent=True) or {}      
    ...
    if len(title) > 150:
    return jsonify({"message": "Title must be 150 characters or less"}), 400
    ...
    # try/finally ensures connection and cursor are closed even if query or response prepration fails
    # Otherwise error could leave it open
    try:
        # Save the list for logged-in user
        cur.execute(
        ...
        # Get complete saveed list for response
        cur.execute(
                "SELECT id, user_id, title, description, created_at"
                "FROM Lists WHERE id = %s",
                (list_id,),
